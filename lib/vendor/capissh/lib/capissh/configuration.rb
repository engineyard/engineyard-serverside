...
module Capissh
  ...
  class Configuration
    ...
    def self.pre_exec_callback
      proc do |command, server|
        command.gsub(/\$CAPISSH:HOST\$/, server.host)
      end
    end

    attr_accessor :debug, :logger, :dry_run, :preserve_roles, :options, :pre_exec_callback
    ...
    def initialize(options={})
      ...
      @default_environment = {}
      @default_run_options = {}
      @pre_exec_callback = @options.delete(:pre_exec_callback) || self.class.pre_exec_callback
    end
    ...
    def run_tree(servers, tree, options={}) #:nodoc:
      ...
      connections.execute_on_servers(servers, options) do |sessions| # FIXME: leaky abstraction, execute_on_servers can't be accessed here
